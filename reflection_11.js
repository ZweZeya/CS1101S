        ...
        if (is_literal(command)) {
            ...
            C = pair(conditional_predicate(command),
                  pair(make_branch_instruction(
                    ...
                    C));
        } else if (is_logical_composition(command)) { // logical composition
            const sym = logical_symbol(command);
            const exp1 = logical_composition_first_component(command);
            const exp2 = logical_composition_second_component(command);
            C = sym === "&&"
                ? pair(make_conditional_expression(
                    exp1, exp2, 
                    make_literal(false)), C)
                : pair(make_conditional_expression(
                    exp1, make_literal(true), exp2), C);
...
function apply_binary(operator, op1, op2) {
    return operator === "+"
           ...
           : operator === "%" 
        //   ? op1 % op2
           ? math_sqrt(op1 * op1 + op2 * op2) // hypothenuse operator
           : operator === "<" 
           ? op1 < op2
           : operator === ">" 
           ? op1 > op2
...
// parse_and_evaluate("true ? true : false;");
// parse_and_evaluate("false && true;");
parse_and_evaluate(`parse("1;");`);
// parse_and_evaluate("math_pow(2,2);");